...
def _dpmeans_single_lloyd(
    ...
    with threadpool_limits(limits=1, user_api="blas"):
        for i in range(max_iter):
            ...
            if max_clusters == centers.shape[0]:
                _, counts = np.unique(labels, return_counts=True)
                new_small_clusters = np.sum(counts <= 10)
                new_small_clusters = new_small_clusters - small_clusters
                small_clusters += new_small_clusters
                max_clusters += new_small_clusters
