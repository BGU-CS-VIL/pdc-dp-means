...
metrics_dict = {NMI: lambda pred,gt,data:nmi(gt,pred),
...
def train_val_test_split(data,labels,val_frac,test_frac):
    X_train, X_test, y_train, y_test = train_test_split(data, labels, test_size=test_frac, random_state=1)
    X_train, X_val, y_train, y_val = train_test_split(X_train, y_train, test_size=val_frac, random_state=1) # 0.25 x 0.8 = 0.2
    return (X_train,y_train),(X_val,y_val),(X_test,y_test)


def calc_objective(model,obj,data,gt):
    return -metrics_dict[obj](pairwise_distances_argmin(data, model.cluster_centers_),gt,data)
