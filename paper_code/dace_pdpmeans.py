...
def dpmeans_chunk_ddp_spawn(q,data,delta):
    dpmeans = DPMeans(init="k-means++", n_clusters=1, n_init=1, delta=delta,max_iter=500).fit(data).cluster_centers_
    q.put(dpmeans)
...
def DACE_DDP(data,delta,num_of_processes):
    ...
    for i in range(num_of_processes):
        ...
        cur_indices = t.get_nns_by_item(min_index,data.shape[0], include_distances=False)
        cur_size = 0        
    ...
    for i in range(num_of_processes):
        ...
        p = Process(target = dpmeans_chunk_ddp, args = (q,data[labels==i],delta,))
        p.start()
        ps.append(p)
    for i in range(num_of_processes):        
...
def DACE_DDP_SPAWN(data,delta,num_of_processes):
    ...
    q = Queue()
    for i in range(num_of_processes):
        p = ctx.Process(target = dpmeans_chunk_ddp_spawn, args = (q,data[labels==i],delta,))
        p.start()
        ps.append(p)
    for i in range(num_of_processes):        
        centers.append(q.get())
...
def DACE(data,delta,num_of_processes):
    ...
    centers = []
    for i in range(num_of_processes):
        p = Process(target = dpmeans_chunk, args = (q,data[labels==i],delta,))
        p.start()
        ps.append(p)
    for i in range(num_of_processes):        
        centers.append(q.get())
...
def ddp_nopar_forp(q_centers,q_labels,data,centers, delta ,start_index):    
    ...
    prev_obj = calc_dpmeans_objective(centers,data,delta)/(D*N)
    min_arg, min_dist = pairwise_distances_argmin_min(data,centers)
    q_labels.put((start_index,min_arg,))
    max_index = np.argmax(min_dist)
    max_dist = min_dist[max_index]
    labels = min_arg
...
def parallel_dp_batch(q_centers,q_labels,data,centers,delta,label_start_index):
    ...
    for i in range(data.shape[0]):
        if min_dist[i]**2 > delta:
            if len(new_centers) > 0:
                new_min_arg, new_min_dist = pairwise_distances_argmin_min(data[i].reshape(1,-1),new_centers)
    ...
    for i in range(len(new_centers)):
        center_candidates.append((new_center_labels[i],new_centers[i],))
    q_labels.put((label_start_index,min_arg,))
...
def dpvalidate(centers,max_cluster,delta):
    ...
    for i in range(1,len(centers)):
        min_arg, min_dist = pairwise_distances_argmin_min(centers[i][1].reshape(1,-1),new_centers)
        if min_dist**2 > delta:
    ...
    for i in range(iters):
        ...
        if processes > 1:
            for j in range(processes):
                p = Process(target = parallel_dp_batch, args = (centers_q,
                                                                labels_q,
                                                                chunks[j],
                                                                centers,
                                                                delta,
        ...
        if len(new_labels) > 0:
            ...
            label_indices = sum([new_centers[i][0] for i in range(len(new_centers))],[])
            for i in range(len(new_centers)):
